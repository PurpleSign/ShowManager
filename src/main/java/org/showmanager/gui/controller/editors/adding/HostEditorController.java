...
public class HostEditorController extends BasicAddingEditorController {
	@FXML
	private TextField tfHostName, tfHostURL;
...
//--------------------------------------------------------------------------------------------------------
	...
	public void progress() {
		if (isInputValid()) {
			try {
				if (editObjectID == -1) showManager.addHost(tfHostName.getText(), tfHostURL.getText());
				else {
					showManager.setHostName(editObjectID, tfHostName.getText());
					showManager.setHostURL(editObjectID, tfHostURL.getText());
				}
...
//--------------------------------------------------------------------------------------------------------
	...
	protected boolean isInputValid() {
		...
		if ((tfHostName.getText() != "") && (tfHostURL.getText() != "")) {
			vRet = true;
			for (Integer vGenreID : showManager.getGenreIDs()) {
				try {	if (showManager.getGenreName(vGenreID).equals(tfHostName.getText()) 
